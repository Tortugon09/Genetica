...
def start_training():
    ...
    if file_path:  # Verificar si se ha seleccionado un archivo
...
def generate_report():
    ...
    # Asegurarse de que el widget de texto esté limpio antes de insertar el nuevo reporte
...
root.minsize(400, 750)  # Tamaño mínimo de la ventana


# Estilo y tema
style = ttk.Style()
style.theme_use('alt')  # Cambiar a un tema diferente si se desea
...
report_frame.pack(fill="both", expand=True, padx=20, pady=10)  # Se expande con la ventana

# Widget Text para mostrar el reporte
report_text = tk.Text(report_frame, wrap="word", font=font, bg=bg_color, fg=text_color)
report_text.pack(expand=True, fill="both", padx=10, pady=10)  # Llena el espacio disponible
report_text.config(state="disabled")  # Inicia deshabilitado para que no sea editable


# Widgets dentro del labelframe
...
# Frame para mostrar el nombre del archivo seleccionado
